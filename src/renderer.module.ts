...
export class Renderer {
  ...
  drawSnake(snake: Position[], ground: Grid[][], color = 'black') {
    this.ctx.save()
    
    this.ctx.fillStyle = color
    let selectedGrid = null
    for (const position of snake) {
      selectedGrid = ground[position.x][position.y]
      this.ctx.fillRect(selectedGrid.topLeftPoint.x, selectedGrid.topLeftPoint.y, selectedGrid.sideLength, selectedGrid.sideLength)
    }
